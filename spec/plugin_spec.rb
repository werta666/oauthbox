...
describe OauthboxAuthenticator do
  describe "after_authenticate" do
    before { SiteSetting.oauthbox_user_json_url = "https://provider.com/user" }

    let(:user) { Fabricate(:user) }
    let(:authenticator) { OauthboxAuthenticator.new }
    ...
    before(:each) { SiteSetting.oauthbox_email_verified = true }
    ...
    it "validates user email if provider has verified" do
      SiteSetting.oauthbox_email_verified = false
    ...
    describe "fetch_user_details" do
      before(:each) do
        SiteSetting.oauthbox_fetch_user_details = true
        SiteSetting.oauthbox_user_json_url = "https://provider.com/user"
        SiteSetting.oauthbox_user_json_url_method = "GET"
        SiteSetting.oauthbox_json_email_path = "account.email"
      end
      ...
      it "works" do
        stub_request(:get, SiteSetting.oauthbox_user_json_url).to_return(success_response)
        result = authenticator.after_authenticate(auth)
        expect(result.email).to eq("newemail@example.com")

        SiteSetting.oauthbox_user_json_url_method = "POST"
        stub_request(:post, SiteSetting.oauthbox_user_json_url).to_return(success_response)
        result = authenticator.after_authenticate(auth)
        expect(result.email).to eq("newemail@example.com")
      end

      it "returns an standardised result if the http request fails" do
        stub_request(:get, SiteSetting.oauthbox_user_json_url).to_return(fail_response)
        result = authenticator.after_authenticate(auth)
        expect(result.failed).to eq(true)

        SiteSetting.oauthbox_user_json_url_method = "POST"
        stub_request(:post, SiteSetting.oauthbox_user_json_url).to_return(fail_response)
        result = authenticator.after_authenticate(auth)
        expect(result.failed).to eq(true)
      end

      describe "fetch custom attributes" do
        after { DiscoursePluginRegistry.reset_register!(:oauthbox_additional_json_paths) }
        ...
        it "stores custom attributes in the user associated account" do
          ...
          DiscoursePluginRegistry.register_oauthbox_additional_json_path(
            custom_path,
            Plugin::Instance.new,
          )
          stub_request(:get, SiteSetting.oauthbox_user_json_url).to_return(response)
      ...
      describe "required attributes" do
        after { DiscoursePluginRegistry.reset_register!(:oauthbox_required_json_paths) }
  ...
  it "can walk json" do
    authenticator = OauthboxAuthenticator.new
    json_string = '{"user":{"id":1234,"email":{"address":"test@example.com"}}}'
    SiteSetting.oauthbox_json_email_path = "user.email.address"
  ...
  it "allows keys containing dots, if wrapped in quotes" do
    ...
    SiteSetting.oauthbox_json_user_id_path = '"www.example.com/uid"'
  ...
  it "allows keys containing literal backslashes, if escaped" do
    ...
    _
    SiteSetting.oauthbox_json_user_id_path = <<~'_'.chomp
      www\.example\.com/uid\\
    _
    result = authenticator.json_walk({}, JSON.parse(json_string), :user_id)
    expect(result).to eq "myuid"
  ...
  it "can walk json that contains an array" do
    ...
    SiteSetting.oauthbox_json_user_id_path = "identities.[].user_id"
    result = authenticator.json_walk({}, JSON.parse(json_string), :user_id)
  ...
  it "can walk json and find values by index in an array" do
    ...
    SiteSetting.oauthbox_json_email_path = "emails[1].value"
    result = authenticator.json_walk({}, JSON.parse(json_string), :email)
  ...
  it "can walk json and download avatar" do
    ...
    SiteSetting.oauthbox_json_avatar_path = "user.avatar"
  ...
  it "can walk json and appropriately assign a `false`" do
    ...
    SiteSetting.oauthbox_json_email_verified_path = "user.data.is_cat"
    result =
  ...
  describe "token_callback" do
    let(:user) { Fabricate(:user) }
    let(:strategy) { OmniAuth::Strategies::Oauthbox.new({}) }
    let(:authenticator) { OauthboxAuthenticator.new }
    ...
    before(:each) do
      SiteSetting.oauthbox_callback_user_id_path = "params.info.uuid"
      SiteSetting.oauthbox_callback_user_info_paths = "name:params.info.name|email:params.info.email"
    end
